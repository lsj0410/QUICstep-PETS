...
R"_d3l1m1t3r_(    'user_switch', AndroidMultiuserMetric_EventData(
        'duration_ms', (
...
R"_d3l1m1t3r_(-- They are computed separately as latency ones are slightly different, don't
...
R"_d3l1m1t3r_(FROM android_process_metadata
...
R"_d3l1m1t3r_(              WHERE proc_start_ts > (SELECT COALESCE(MIN(ts), 0)
...
R"_d3l1m1t3r_(              WHERE gc_type = "native_alloc"
            ),
...
R"_d3l1m1t3r_(              WHERE gc_ts > (SELECT COALESCE(MIN(ts), 0)
                FROM thread_slice WHERE name GLOB "*android.intent.action.USER_UNLOCKED*"
                ORDER BY ts ASC LIMIT 1 )
            ),
            'num_of_threads_with_gc', (SELECT SUM(cnt) FROM (SELECT COUNT(*) AS cnt
...
R"_d3l1m1t3r_(                WHERE name GLOB "*android.intent.action.USER_UNLOCKED*" ORDER BY ts
                ASC LIMIT 1 )
            ),
            'full_gc_count', (SELECT COUNT(*) FROM android_garbage_collection_events
...
R"_d3l1m1t3r_(    launches.startup_id = $startup_id
);
...
R"_d3l1m1t3r_(          "destination_process", s.process,
...
R"_d3l1m1t3r_(        'unknown', mcycles_for_launch_and_core_type(launches.startup_id, 'unknown')
      )),
      'to_post_fork',
...
R"_d3l1m1t3r_(      'time_activity_resume',
...
R"_d3l1m1t3r_(      'time_dlopen_thread_main',
      dur_sum_main_thread_slice_proto_for_launch(
        launches.startup_id,
        'dlopen:*.so'),
...
R"_d3l1m1t3r_(        FROM running_gc_slices_materialized
...
R"_d3l1m1t3r_(        WHERE h.id =launches.startup_id
      )
    )),
...
R"_d3l1m1t3r_(      'installd_running',
...
R"_d3l1m1t3r_(          main_thread_time_for_launch_in_runnable_state(launches.startup_id) > launches.dur * 0.15
...
R"_d3l1m1t3r_(        WHERE android_sum_dur_for_startup_and_slice(launches.startup_id, 'inflate') > 450e6
...
R"_d3l1m1t3r_(WITH actual_timeline_with_vsync AS (
  SELECT
    *,
...
R"_d3l1m1t3r_(  -- for a given vsync but using MAX here in case this changes in the future.
...
R"_d3l1m1t3r_(CREATE PERFETTO TABLE android_jank_cuj_layer_name AS
SELECT
    cuj_id,
...
R"_d3l1m1t3r_(  FROM android_jank_cuj_do_frame_slice do_frame
...
R"_d3l1m1t3r_(    AND expected_timeline.name = actual_timeline.name;
...
R"_d3l1m1t3r_(  'table_name', 'shmem_rss',
...
R"_d3l1m1t3r_(  shmem_rss_span PARTITIONED upid
);
...
R"_d3l1m1t3r_(    || get_longest_chunk(slice_ts, slice_dur, tid, name)
    || " [ extra_info: "
    || " launches_dur_ms " || get_ns_to_ms($launches_dur)
    || " sum_dur_ms "
...
R"_d3l1m1t3r_(          android_sum_dur_for_startup_and_slice(launch.startup_id, 'VerifyClass*')
            > launch.dur * 0.15
...
R"_d3l1m1t3r_(          get_main_thread_binder_transactions_blocked(launch.startup_id, 2e7) as details
        FROM android_startups launch
        WHERE launch.startup_id = $startup_id
          AND (
          SELECT COUNT(1)
