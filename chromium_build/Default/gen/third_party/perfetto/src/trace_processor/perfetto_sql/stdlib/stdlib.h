...
R"_d3l1m1t3r_(  track_name STRING,
...
R"_d3l1m1t3r_(-- (everything after the '=') to construct a slice. It computes the timestamp
...
R"_d3l1m1t3r_(      LEAD(ts) OVER (PARTITION BY track_name, key ORDER BY ts) AS next_ts
    FROM event_markers
  ),
...
R"_d3l1m1t3r_(FROM event_spans;
...
R"_d3l1m1t3r_(  ON all_binder.server_upid = client_process_metadata.upid;
...
R"_d3l1m1t3r_(  extract_arg(arg_set_id, 'packet_transport') AS packet_transport,
...
R"_d3l1m1t3r_(    WHEN $value = -900 THEN 'system'
...
R"_d3l1m1t3r_(    ELSE 'unknown_app'
  END;
...
R"_d3l1m1t3r_(  oom_adj_reason STRING,
...
R"_d3l1m1t3r_(  reason.oom_adj_reason,
...
R"_d3l1m1t3r_(--       over all iterations you get the final Speedometer score for the run.
...
R"_d3l1m1t3r_(  sync_end_ts - start_ts AS dur
FROM filtered;
...
R"_d3l1m1t3r_(-- where each subphase can happen only once.
...
R"_d3l1m1t3r_(          "FIRST_GESTURE_SCROLL_UPDATE",
...
R"_d3l1m1t3r_(  events.event_latency_id,
  events.dur,
...
R"_d3l1m1t3r_(  frames.event_latency_id,
  frames.dur,
...
R"_d3l1m1t3r_(  LAG(presentation_timestamp, 1, presentation_timestamp)
...
R"_d3l1m1t3r_(  delay_since_last_frame INT,
  -- Event latency id of the presented frame.
  event_latency_id INT,
...
R"_d3l1m1t3r_(  presentation_timestamp INT
) AS
SELECT DISTINCT
presentation_timestamp
...
R"_d3l1m1t3r_(  scroll_jank_causes BYTES
) AS
SELECT
  scroll_id,
...
R"_d3l1m1t3r_(    FROM $counter_table
  )
  SELECT
    id,
    ts,
...
R"_d3l1m1t3r_(-- The search is bounded by the sum of edge weights on the path and the root node specifies the
...
R"_d3l1m1t3r_(--        FROM _wakeup_chain
--      ),
--      (
--        SELECT
--          id AS root_node_id,
...
R"_d3l1m1t3r_(  -- roots of the reachability trees. This table must have the columns
...
R"_d3l1m1t3r_(-- |root_node_id| is the id of the starting node under which this edge was encountered.
-- |node_id| is the id of the node from the input graph and |parent_node_id|
-- is the id of the node which was the first encountered predecessor in a DFS
-- search of the graph.
RETURNS TableOrSubquery AS
...
R"_d3l1m1t3r_(    'java.lang.ref.FinalizerReference'
  )
  UNION ALL
...
R"_d3l1m1t3r_(  (SELECT memory_heap_graph_super_root_fn()) as source_node_id,
  id AS dest_node_id
FROM heap_graph_object
WHERE root_type IS NOT NULL;
...
R"_d3l1m1t3r_(  -- Total self_size of all objects dominated by this object, self inclusive.
...
const char kSchedUtilizationGeneral[] = R"_d3l1m1t3r_(--
-- Copyright 2024 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
const char kSchedUtilizationProcess[] = R"_d3l1m1t3r_(--
-- Copyright 2024 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(CREATE PERFETTO FUNCTION sched_process_utilization_per_second(
  -- Upid of the process.
  upid INT
)
RETURNS TABLE (
...
const char kSchedUtilizationSystem[] = R"_d3l1m1t3r_(--
-- Copyright 2024 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
const char kSchedUtilizationThread[] = R"_d3l1m1t3r_(--
-- Copyright 2024 The Android Open Source Project
--
-- Licensed under the Apache License, Version 2.0 (the "License");
-- you may not use this file except in compliance with the License.
...
R"_d3l1m1t3r_(-- a human-readable string.
...
R"_d3l1m1t3r_(    -- the sub-queries were 'unioned'.
...
R"_d3l1m1t3r_(    WHERE critical_path_start_depth.critical_path_utid = $critical_path_utid AND spans.critical_path_utid != spans.utid
  ),
...
R"_d3l1m1t3r_(  cpu_start_depth_raw AS (
    SELECT critical_path_utid, ts, MAX(stack_depth) + 1 AS start_depth
...
R"_d3l1m1t3r_(-- @column track_id           Alias for `slice.track_id`.
...
R"_d3l1m1t3r_(      parent.name as name,
      current.track_id,
...
const FileToSql kFileToSql[] = {
  ...
  {"sched/utilization/general.sql", kSchedUtilizationGeneral},

  {"sched/utilization/process.sql", kSchedUtilizationProcess},

  {"sched/utilization/system.sql", kSchedUtilizationSystem},

  {"sched/utilization/thread.sql", kSchedUtilizationThread},
